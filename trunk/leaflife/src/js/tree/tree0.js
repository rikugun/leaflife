...
Tree.prototype =
{
    ...
    {
        var result;
        if (! nodeObj[eventType])  // prevent before event from triggering multiple times
        {
    ...
    destructor: function()
    {
        Tree.txtbox = Tree.xmlParser = Tree.dragHint = Tree.droppableSibling = null;
 ...
 * Tree Events
 *****************************************************************************/
Tree.captureEvent = function(event)
{
...
//alert("blurOnEditbox()");
    ...
    {
        nodeObj.setText(this.value);  // save changes
    }
}
...
Tree.mouseDownOnNode = function(event)
{
    ...
    Tree.captureEvent(event);
    treeObj.cancelEdit();
    if (treeObj._9 && event.which == 1)  // left button
    {
        Tree.dragging = false;
        Tree.nodeDragged = nodeObj;
        document.onmouseup = Tree.mouseUpOnPage;
        document.onmousemove = Tree.mouseMoveOnPage;
    }
}

Tree.mouseMoveOnPage = function(event)
{
    var srcElem = event.target, dropEffect = srcElem.dropEffect, doc = document, body = doc.body, dragHint = Tree.dragHint, dragStyle;
    if (dropEffect && dropEffect != "none")
    {
        var srcNode = Tree.nodeDragged, droppableSibling = Tree.droppableSibling, tgtNode, tgtTree, posPreviousSibling, linkObj;
        if (Tree.dragging)  // dragging
        {
            ...
            else
            {
                if (srcElem.nodeObj)  // drag over on node
                {
                    ...
                    {
                        tgtNode.addTextStyle("droppableText");
                ...
                else  // drag over on td
                {
                    ...
                    {
                        tgtNode.treeObj._18.insertBefore(droppableSibling, posPreviousSibling ? tgtNode.navObj : tgtNode.navObj.nextSibling);
                        droppableSibling.className = "droppableSibling";
                    }
                    srcElem.onmouseout = Tree.dragLeaveOnSibling;
                }
            }
        }
        else if (srcNode.treeObj.fireEvent("dragstart", true, srcNode))  // drag start
        {
            body.style.cursor = "no-drop";
            srcNode.addTextStyle("draggedText");
            if (! droppableSibling)
            {
...
Tree.mouseUpOnPage = function(event)
{
    ...
    if (Tree.dragging)
    {
        var srcElem = event.target, defaultDropEffect = srcElem.dropEffect, nodeDragged = Tree.nodeDragged, dragHint = Tree.dragHint, droppableSibling = Tree.droppableSibling;
        doc.body.style.cursor = "";
        nodeDragged.removeTextStyle("draggedText");
        dragHint.className = droppableSibling.className = "hidden";
        if (defaultDropEffect)
        {
            srcElem.style.cursor = "";
            if (droppableSibling.dropEffect != "none")
            {
                var tgtNode = srcElem.nodeObj, linkObj = srcElem.firstChild, position = "lastChild", dropEffect, tgtTree, newNode;
                ...
                tgtTree = tgtNode.treeObj;
                dropEffect = tgtTree.fireEvent("drop", defaultDropEffect, nodeDragged, tgtTree, tgtNode, position);
                if (dropEffect == "move")
                {
                    newNode = nodeDragged.moveTo(tgtNode, position);
                }
                else if (dropEffect == "copy")
                {
                    newNode = nodeDragged.copyTo(tgtNode, position);
                }
                if (newNode)
                {
                    newNode.select();
...
//        alert(this.id + ".draw(" + insertAtObj + ")");
        ...
        anchorObj.dropEffect = cellObj.dropEffect = treeObj._10;
        ...
        if (nodeObj.treeObj._0)
        {
            ...
            while (nodeObj)
            {
                for (i = nodeObj.nChildren - 1; i >= 0; -- i)
                {
                    ...
                    childNode.navObj.rows[0].cells[pos].className = code ? "imgLine4" : "";
                    nodes[len ++] = childNode;
                }
                nodeObj = nodes[-- len];
            }
...
//        alert(this + ".detachFromParent()");
    ...
    {
        var txtbox = Tree.txtbox, treeObj = this.treeObj, linkObj = this.linkObj;
        if (linkObj && linkObj.contentEditable != "true" && treeObj.fireEvent("beforeedit", true, this))
        {
            if (! txtbox)
            {
                Tree.txtbox = txtbox = document.createElement("input");
                txtbox.type = "text";
                txtbox.className = "nodeText editbox";
            ...
            txtbox.size = (txtbox.value = this.text).replace(/[^\x00-\xff]/g, "**").length;
...
//alert(this.text + ".expand()");
    ...
    select: function(preserveSelected)
    {
        ...
        if (! nodeSelected.containsKey(nodeId) && treeObj.fireEvent("beforeselectchange", true, this, true) && ! nodeSelected.containsKey(nodeId))
        {
            ...
            this.setIconStyle();
            var treeNavObj = treeObj._18, navObj = this.navObj, linkObj = this.linkObj, scrollTop = treeNavObj.scrollTop, scrollHeight = scrollTop + treeNavObj.clientHeight;
...
//        alert(this.text + ".copyTo(" + tgtNodeObj + "," + mode + ")");
    ...
    {
        var srcNodeObj = Tree.nodeDragged, tgtTreeObj = this.treeObj, dropEffect = tgtTreeObj._10, srcElem = event.target, droppableSibling = Tree.droppableSibling, cursor = "default";
        if (srcNodeObj)
        {
